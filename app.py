...
def reg():
    ...
    if(cursor.execute("INSERT INTO users(user_name,user_id, password) VALUES(%s,%s,%s)",(user_name,user_id,password))):
...
def reg():
    ...
    # form = RegistrationForm(request.form)
        ...
        if request.method == "POST":
            ...
            print(user_name,id,user_id, password)
            conn=mysql.connect()
            cursor = conn.cursor()

            x = cursor.execute("SELECT * FROM users WHERE user_name = (%s) and user_id =(%s)",[user_name,user_id])
            ...
            if int(x) > 0:
                ...
                if(cursor.execute("INSERT INTO users (user_name, password, user_id) VALUES (%s, %s, %s)",(user_name, password, user_id))):
...
def send_message():
    if request.method == 'POST':
        ...
        account_sid = 'ACe1de668f66b3cf5e4729f17436df1949' 
        auth_token = '0e8e94459978315aab0bf41647eb9953' 
        client = Client(account_sid, auth_token) 
 
        message = client.messages.create( 
                              from_='whatsapp:+14155238886',  
                              body= main_msg,      
                              to='whatsapp:+919611379774'
                          ) 
